...
def prepare_data_commit_guru_file(project):
    ...
    commit_data_df = commit_data_df[['commit_hash','created_at']]
...
def label_propagation(X_train, y_train):
    label_prop_model = LabelPropagation(max_iter=4000)
...
def run_classifiers(project, supervised_model_list):
    ...
    train_df,test_df,success  = prepare_data_commit_guru_file(project)
    ...
    for release in test_df.release.unique():
        ...
        X_test = test_df.drop(['Bugs', 'LOC'], axis = 1)
        ...
        for model in supervised_model_list.keys():
            ...
            if first_build:
                ...
                clf = self_training(clf, X_train_mixedlabeled, y_train_mixedlabeled)
        ...
        if first_build:
            label_prop_model = label_propagation(X_train_mixedlabeled, y_train_mixedlabeled)
        ...
        if first_build:
            label_spread_model = label_spreading(X_train_mixedlabeled, y_train_mixedlabeled)
        ...
        if first_build:
            gm, label = semi_GMM(X_train_mixedlabeled,y_train_mixedlabeled,X_train_labeled,y_train_labeled)
            models['Semi_GMM'] = [gm, label]
        else:
            gm, label = models['Semi_GMM'][0], models['Semi_GMM'][1]
        predicted = semi_GMM_predict(gm, label, X_test)
    ...
    result_path = 'results/Project_specific_release/' + project + '.pkl'
...
if __name__=="__main__":
    ...
    run(existing_projects, supervised_model_list) 
    # threads = []
    # cores = cpu_count()
