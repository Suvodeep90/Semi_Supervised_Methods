...
def load_data(project):
    train_df,_,success  = prepare_data_commit_guru_file(project)#,'process')
    _,test_df,success  = load_data_merged(project,'process')
...
def prepare_data_commit_guru_file(project):
    ...
    release_data_path = '../all_data/defect_prediction/700/release/' + project + '_release.pkl'
...
#     data_df = data_df[data_df.commit_hash.isin(commits)]
...
#     train_df = train_df[train_df.commit_hash.isin(commits)]
...
def load_data_merged(project, metric):
    path = '../all_data/defect_prediction/700/merged_data/' + project + '.csv'
    df = pd.read_csv(path)
    ...
    for release in releases:
        if start >= 3:
            break
        _test_df = df[df['release'] == release]
        if (_test_df.shape[0]) > 10 and (len(_test_df.Bugs.unique()) == 2):
            acceped_release.append(release)
            start += 1
    
    train_df = df[df.release.isin(releases[3:])]
    test_df = df[df.release.isin(acceped_release)]
    
    train_df = train_df[['file_la', 'file_ld', 'file_lt', 'file_age', 'file_ddev',
       'file_nuc', 'own', 'minor', 'file_ndev', 'file_ncomm', 'file_adev',
       'file_nadev', 'file_avg_nddev', 'file_avg_nadev', 'file_avg_ncomm',
       'file_ns', 'file_exp', 'file_sexp', 'file_rexp', 'file_nd', 'file_sctr','Bugs']]
    
    LOC = train_df['file_la'] + train_df['file_lt'] - train_df['file_ld']
    train_df['LOC'] = LOC
    
    test_df = test_df[['file_la', 'file_ld', 'file_lt', 'file_age', 'file_ddev',
       'file_nuc', 'own', 'minor', 'file_ndev', 'file_ncomm', 'file_adev',
       'file_nadev', 'file_avg_nddev', 'file_avg_nadev', 'file_avg_ncomm',
       'file_ns', 'file_exp', 'file_sexp', 'file_rexp', 'file_nd', 'file_sctr','Bugs','release']]
    
    LOC = test_df['file_la'] + test_df['file_lt'] - test_df['file_ld']
    test_df['LOC'] = LOC
...
def get_data(train_df,X_test,y_test, loc_data, percentage):
    ...
    # for i in range(train_df_bugs.shape[0],0,-1):
    #     if i > train_df_bugs.shape[0] - int(labeled_data_size/2):
    #         train_df_bugs.loc[i-1,'Bugs'] = train_df_bugs.loc[i-1,'Bugs']
    #     else:
    #         train_df_bugs.loc[i-1,'Bugs'] = -1

    # for i in range(train_df_non_bugs.shape[0],0,-1):
    #     if i > train_df_non_bugs.shape[0] - int(labeled_data_size/2):
    #         train_df_non_bugs.loc[i-1,'Bugs'] = train_df_non_bugs.loc[i-1,'Bugs']
    #     else:
    #         train_df_non_bugs.loc[i-1,'Bugs'] = -1
    ...
    # unlabeled_y = []
    # actual_p = 1 - (percentage/100)
    # if X_train.shape[0]*(1-actual_p) < 10:
    #     actual_p = 1 - 10/y_train.shape[0]
    # # print(actual_p)
    # for y in y_train:
    #     if np.random.binomial(1, p=actual_p) == 1:
    ...
    # print(X_train.shape, labeled_df.shape, unlabeled_df.shape)
    ...
    # print('before', X_train_labeled.shape)
...
def run_classifiers(project, supervised_model_list, percentage):
    ...
    if not success:
        ...
        # print(project, success, train_df.Bugs.unique(), test_df.Bugs.unique(), test_df.release.unique())
    ...
    for release in test_df.release.unique():
        ...
        # train_loc = train_df.LOC
        # X_train = train_df.drop(['Bugs', 'LOC'], axis = 1)
